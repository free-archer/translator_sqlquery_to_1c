...
Процедура ЗаменитьПоляSQL(Текст1С, БазаДанных, ТаблицаСоответствия)
	...
	Шаблон= Псевдоним+"\._\w+";
	мНайденыхСтрок= РегулярныеВыражения.RegExp(Шаблон, Текст1С, Истина, Истина, Истина, Истина); 
		
		Для Каждого Строка1С Из мНайденыхСтрок Цикл 
		Пока Выборка.Следующий() Цикл                          
			Если Не ПустаяСтрока(Выборка.Наименование) Тогда
				Текст1С= СтрЗаменить(Строка1С, Псевдоним+Выборка.ИмяХранения, Псевдоним+Выборка.Наименование);
				Прервать;
			КонецЕсли;
		КонецЦикла; 
		КонецЦикла;
